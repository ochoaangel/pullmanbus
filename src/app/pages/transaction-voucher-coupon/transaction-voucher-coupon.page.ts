...
@Component({
  selector: 'app-transaction-voucher-coupon',
  templateUrl: './transaction-voucher-coupon.page.html',
  styleUrls: ['./transaction-voucher-coupon.page.scss'],
})
export class TransactionCouponVoucherPage implements OnInit {
  ...
  constructor(
    ...
    this.loading += 1;
    this.integradorService.buscarEncabezadoCuponera({ "orden": this.codigo }).subscribe((resp: any) => {
      this.loading -= 1;
  ...
  btnDescargaPasaje(boleto: string, codigo: string) {
    ...
    this.loading += 1;
    this.integradorService.generarComprobanteCuponera(this.postComprobante).subscribe(resp => {
      this.respPDF = resp;
      this.loading -= 1;
      this.downloadPDF();
    });
